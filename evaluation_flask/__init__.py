...
def profil():
    ...
    for u in users_rows:
        u_dict = dict(u)
        u_dict['avatar_url'] = u_dict['avatar_url'] if u_dict['avatar_url'] else 'default.png'
    ...
    for post in posts_rows:
        ...
        cursor.execute('''
            SELECT c.id, c.content, c.created_at, u.username, u.avatar_url, COUNT(cl.id) AS likes_count
            FROM comments c
    ...
    for abonne in cursor.fetchall():
        ...
        ab_dict['avatar_url'] = ab_dict['avatar_url'] if ab_dict['avatar_url'] else 'default.png'
    ...
    for abo in cursor.fetchall():
        ...
        abo_dict['avatar_url'] = abo_dict['avatar_url'] if abo_dict['avatar_url'] else 'default.png'
    ...
    if user_row:
        ...
        user_dict['avatar_url'] = user_dict['avatar_url'] if user_dict['avatar_url'] else 'default.png'
        user_dict['bio'] = user_dict['bio'] if user_dict['bio'] else ""
        user_dict['streak_days'] = user_row['streak_days'] if 'streak_days' in user_row.keys() else 0
    else:
        user_dict = {
            "id": session['user_id'],
...
def profil_public(user_id):
    ...
    user_dict['border_color'] = user_dict.get('border_color', '#4CAF50')  # Cadre couleur uniquement pour profil principal
